# Avoid multiple calls to find_package to append duplicated properties to the targets
include_guard()########### VARIABLES #######################################################################
#############################################################################################
set(libiconv_FRAMEWORKS_FOUND_DEBUG "") # Will be filled later
conan_find_apple_frameworks(libiconv_FRAMEWORKS_FOUND_DEBUG "${libiconv_FRAMEWORKS_DEBUG}" "${libiconv_FRAMEWORK_DIRS_DEBUG}")

set(libiconv_LIBRARIES_TARGETS "") # Will be filled later


######## Create an interface target to contain all the dependencies (frameworks, system and conan deps)
if(NOT TARGET libiconv_DEPS_TARGET)
    add_library(libiconv_DEPS_TARGET INTERFACE IMPORTED)
endif()

set_property(TARGET libiconv_DEPS_TARGET
             APPEND PROPERTY INTERFACE_LINK_LIBRARIES
             $<$<CONFIG:Debug>:${libiconv_FRAMEWORKS_FOUND_DEBUG}>
             $<$<CONFIG:Debug>:${libiconv_SYSTEM_LIBS_DEBUG}>
             $<$<CONFIG:Debug>:>)

####### Find the libraries declared in cpp_info.libs, create an IMPORTED target for each one and link the
####### libiconv_DEPS_TARGET to all of them
conan_package_library_targets("${libiconv_LIBS_DEBUG}"    # libraries
                              "${libiconv_LIB_DIRS_DEBUG}" # package_libdir
                              "${libiconv_BIN_DIRS_DEBUG}" # package_bindir
                              "${libiconv_LIBRARY_TYPE_DEBUG}"
                              "${libiconv_IS_HOST_WINDOWS_DEBUG}"
                              libiconv_DEPS_TARGET
                              libiconv_LIBRARIES_TARGETS  # out_libraries_targets
                              "_DEBUG"
                              "libiconv"    # package_name
                              "${libiconv_NO_SONAME_MODE_DEBUG}")  # soname

# FIXME: What is the result of this for multi-config? All configs adding themselves to path?
set(CMAKE_MODULE_PATH ${libiconv_BUILD_DIRS_DEBUG} ${CMAKE_MODULE_PATH})

########## GLOBAL TARGET PROPERTIES Debug ########################################
    set_property(TARGET Iconv::Iconv
                 APPEND PROPERTY INTERFACE_LINK_LIBRARIES
                 $<$<CONFIG:Debug>:${libiconv_OBJECTS_DEBUG}>
                 $<$<CONFIG:Debug>:${libiconv_LIBRARIES_TARGETS}>
                 )

    if("${libiconv_LIBS_DEBUG}" STREQUAL "")
        # If the package is not declaring any "cpp_info.libs" the package deps, system libs,
        # frameworks etc are not linked to the imported targets and we need to do it to the
        # global target
        set_property(TARGET Iconv::Iconv
                     APPEND PROPERTY INTERFACE_LINK_LIBRARIES
                     libiconv_DEPS_TARGET)
    endif()

    set_property(TARGET Iconv::Iconv
                 APPEND PROPERTY INTERFACE_LINK_OPTIONS
                 $<$<CONFIG:Debug>:${libiconv_LINKER_FLAGS_DEBUG}>)
    set_property(TARGET Iconv::Iconv
                 APPEND PROPERTY INTERFACE_INCLUDE_DIRECTORIES
                 $<$<CONFIG:Debug>:${libiconv_INCLUDE_DIRS_DEBUG}>)
    # Necessary to find LINK shared libraries in Linux
    set_property(TARGET Iconv::Iconv
                 APPEND PROPERTY INTERFACE_LINK_DIRECTORIES
                 $<$<CONFIG:Debug>:${libiconv_LIB_DIRS_DEBUG}>)
    set_property(TARGET Iconv::Iconv
                 APPEND PROPERTY INTERFACE_COMPILE_DEFINITIONS
                 $<$<CONFIG:Debug>:${libiconv_COMPILE_DEFINITIONS_DEBUG}>)
    set_property(TARGET Iconv::Iconv
                 APPEND PROPERTY INTERFACE_COMPILE_OPTIONS
                 $<$<CONFIG:Debug>:${libiconv_COMPILE_OPTIONS_DEBUG}>)

########## For the modules (FindXXX)
set(libiconv_LIBRARIES_DEBUG Iconv::Iconv)
