 ...
 * `rgb` is populated by a successful getFrame() call; `depth` is populated
 * only by depth-capable devices (e.g. OAK-D, RealSense). `timestamp` records
 * capture time using the steady clock to avoid wall-clock jumps. `source_id`
 * is a human-readable identifier for the originating device (e.g. "webcam:0",
 * "oak-d", "rtsp://...").
 */
struct CameraFrame {
    std::optional<core::Image<core::BGR>>                rgb;        ///< Colour frame; always populated by a successful getFrame() call.
